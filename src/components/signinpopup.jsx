...
export default function SignInPopup() {
	...
	const { mutate, isPending, isError, isSuccess, error, data, reset } = useMutation({
		mutationKey: [config.endpoints.completeLogin],
		mutationFn: createFetcher({
			method: "POST",
			url: config.endpoints.completeLogin,
		}),
	});
	...
	const {
		...
		mutationKey: [config.endpoints.startLogin],
		mutationFn: createFetcher({
			method: "POST",
			url: config.endpoints.startLogin,
		}),
	});
	...
	useEffect(() => {
		...
		if (isError) {
			snack({
				message: error.message || "Unable to log in, try again",
				variant: "error",
			});
			...
			<div className="fixed inset-0 w-full center z-30">
				<div ref={ref} className="  rounded-[15px] center pt-8 w-[90%] max-w-md bg-[#17082B]/80 backdrop-blur-lg p-6">
					<div className="w-full   lg:w-[95%] min-h-[400px]  flex flex-col justify-start items-center space-y-4  ">
						...
						<p className="text-white/70 text-center text-sm w-[80%]">Sign in to use Tao</p>
								...
								return (
									<Form className="w-full space-y-6">
										...
										<p className="text-white text-center">
											...
											<span onClick={signUpInstead} className="text-[#00E5FF] cursor-pointer">
												{" "}
												Sign up here
											</span>{" "}
										</p>
